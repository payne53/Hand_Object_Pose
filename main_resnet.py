        ...
        if args.val and (epoch+1) % args.val_epoch == 0:
            ...
            with torch.no_grad():
                for v, val_data in tqdm(enumerate(valloader)):
                    ...
                    for j in range(len(outputs2d)):
                        hand_util.feed(labels2d[j, :21], outputs2d[j, :21])
                        obj_util.feed(labels2d[j, 21:], outputs2d[j, 21:])
            ...
            print('mean hand error: {:.5f} pixel'.format(hand_mean_all))
            print('mean object error: {:.5f} pixel'.format(obj_mean_all))
            ...
            for i in range(len(hand_thresholds)):
                print('accuracy @ {} pixel: {:.2f} {:.2f}'.format(hand_thresholds[i], hand_pck_all[i], obj_pck_all[i]))
