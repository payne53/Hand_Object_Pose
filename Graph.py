...
from models.losses import ReProjectionLoss
...
if args.pretrained_graph != '':
    ckpt = torch.load(args.pretrained_graph, map_location='cpu')
    graphunet = {}
    for key, val in ckpt.items():
        if 'graphunet' in key:
            graphunet[key.replace('module.graphunet.', 'module.')] = val
    model.load_state_dict(graphunet)
...
if args.train:
    ...
    for epoch in range(start, args.num_iterations):  # loop over the dataset multiple times
        ...
        for i, tr_data in enumerate(trainloader):
            ...
            # loss = lambda_2 * criterion(outputs3d, labels3d) + lambda_3 * criterion_proj(outputs3d, labels2d)
            # t2 = time.time()
            loss.backward()
            # t3 = time.time()
            optimizer.step()
            ...
            # print('Forward: {:.2f} s'.format(t2 - t1))
            # print('Backward: {:.2f} s'.format(t3 - t2))
            # print('Optimize: {:.2f} s'.format(t4 - t3))
