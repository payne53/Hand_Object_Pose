...
from models.losses import AffinityLoss, CenterLoss
...
if args.pretrained_model != '':
    ...
    for key, val in ckpt.items():
        ...
        if 'interactionnet' in key:
            interactionnet[key.replace('module.interactionnet.', '')] = val
    model.module.resnet.load_state_dict(resnet)
    model.module.graphnet.load_state_dict(graphnet)
    model.module.interactionnet.load_state_dict(interactionnet)

    # model_dict = model.module.interactionnet.state_dict()
    #     # params = interactionnet
    #     # state_dict = {k: v for k, v in params.items() if k in model_dict.keys()}
    #     # model_dict.update(state_dict)
    #     # model.module.interactionnet.load_state_dict(model_dict)

    print("{} loaded.".format(args.pretrained_model))
    # torch.save(model.state_dict(), 'fhad_full_new.pkl')
...
if args.pretrained_graph != '':
    ...
    for key, val in graph_model.items():
        interactionnet[key.replace('module.', '')] = val

    model.module.interactionnet.load_state_dict(interactionnet)
    print('{} loaded.'.format(args.pretrained_graph))
...
if args.train:
    ...
    for epoch in range(start, args.num_iterations):  # loop over the dataset multiple times
        ...
        for i, tr_data in enumerate(trainloader):
            ...
            outputs2d_init, outputs2d, outputs3d_init, outputs3d = model(inputs)

            loss2d_init = criterion(outputs2d_init, labels2d)
            loss2d = criterion(outputs2d, labels2d)
            loss3d_init = criterion(outputs3d_init, labels3d)
            loss3d = criterion(outputs3d, labels3d)

            loss = (lambda_1) * loss2d_init + (lambda_1) * loss2d + (lambda_2) * loss3d_init + (lambda_2) * loss3d
